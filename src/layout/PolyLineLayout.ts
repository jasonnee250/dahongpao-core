...
export class PolyLineLayout extends ILineLayout {
  ...
  layout(
    ...
    if (dx >= 0 && dy >= 0 && dxdy) {
      //第一象限上半区
      GraphicUtils.middleInsertPoint(
        GraphicUtils.topPoint(start),
        GraphicUtils.rightPoint(end),
        points,
      );
    } else if (dx >= 0 && dy >= 0 && !dxdy) {
      //第一象限下半区
      GraphicUtils.doubleMiddleInsertPoint(
        GraphicUtils.leftPoint(start),
        GraphicUtils.rightPoint(end),
        points,
      );
    } else if (dx < 0 && dy > 0 && dxdy) {
      ...
      GraphicUtils.middleInsertPoint(
        GraphicUtils.topPoint(start),
        GraphicUtils.leftPoint(end),
        points,
      );
    } else if (dx < 0 && dy > 0 && !dxdy) {
      //第2象限下半区
      GraphicUtils.doubleMiddleInsertPoint(
        GraphicUtils.rightPoint(start),
        GraphicUtils.leftPoint(end),
        points,
      );
    } else if (dx <= 0 && dy <= 0 && dxdy) {
      //第3象限上半区
      GraphicUtils.middleInsertPoint(
        GraphicUtils.bottomPoint(start),
        GraphicUtils.leftPoint(end),
        points,
      );
    } else if (dx <= 0 && dy <= 0 && !dxdy) {
